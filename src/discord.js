...
function callCmd(cmd, name, client, evt, suffix) {
  ...
  function processEntry(entry) {
    ...
    if (R.is(String, entry) || R.is(Number, entry)) bot.sendText(entry, evt)
      .catch((err, res) => {
        err.res = res;
  ...
  getMessageTTL(user_id).then(exists => {
    ...
    return getUserLang(user_id).then(lang => {
      ...
      if (cmd_return instanceof Promise) {
        ...
        .catch(err => {
          sentry(err, name);
          bot.sendText(`Error: ${err.message}`, evt);
        });
      }
    });
  })
...
function onMessage(evt) {
  ...
  // Checks for PREFIX
  if (evt.object.content[0] === nconf.get('PREFIX')) {
    const command = evt.object.content.toLowerCase().split(' ')[0].substring(1);
    const suffix = evt.object.content.substring(command.length + 2);
    const cmd = commands[command];

    if (cmd) callCmd(cmd, command, {}, evt, suffix);
    return;
  }
...
export function start() {
  ...
  bot.use(new BroidDiscord({token: nconf.get('TOKEN')}));
  bot.use(new BroidSlack({token: nconf.get('SLACK_KEY')}));
  bot.use(new BroidTwitter({
    username: '@Gravebotbot',
    token: nconf.get('TWITTER_TOKEN'),
    tokenSecret: nconf.get('TWITTER_TOKEN_SECRET'),
    consumerSecret: nconf.get('TWITTER_CONSUMER_SECRET'),
    consumerKey: nconf.get('TWITTER_CONSUMER_KEY')
  }));

  Observable.merge(bot.hear('.*', 'Person'), bot.hear('.*', 'Group'))
    .subscribe((data) => onMessage(data.message));
}
