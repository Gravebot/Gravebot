...
export function counters(bot, msg, suffix) {
  ...
  return _makeRequest(options)
    ...
    .then(R.slice(0, 10))
    .then(data => {
      data = R.addIndex(R.map)((champ, idx) => {
      ...
      return phantom('lol_counters', {champs: data}, 700, 400);
    })
...
export function items(bot, msg, suffix) {
  ...
  let position = R.last(suffix_split).toLowerCase();
  let champ = R.join(' ', R.slice(1, -1, suffix_split));
  let champ_reg = champ.replace(/[^a-z]/g, '');
  ...
  return _makeRequest(options)
    ...
    .then(item_data => {
      ...
      item_data.champ_name = toTitleCase(champ);
      item_data.image = champ_reg;
