...
**\'!cat\`
    Retourne une image de chat
**\`!chat\`** \`phrase\`
    Discute avec toi
**\`!coin\`**
    Lance une pièce
**\`!comics\`**
    Obtener un cómic azar de un montón de diferentes artistas
**\`!decide\`** \`quelque chose\` **or** \`quelque chose...\`
    Choisissez entre les mots donnés
**\`!drama\`**
    Répond avec une image dramatique, si aucun numéro est écrit, retourner un aléatoire
**\`!emoji\`**
    Répond avec une copypasta emoji, si aucun numéro est écrit, retourner un aléatoire
**\`!meme\`** \`noms du meme\` \`"texte haut"\` \`"texte bas"\`
    Crée un « meme » avec le texte choisis
**\`!pugbomb\`** \`count\`
    Bombes discuter avec des pugs adorables
**\`!quote\`**
    Répond avec une citation, si aucun numéro est écrit, retourner un aléatoire
**\`!roll\`** \`fois\` \`côtés\`
    Fait rouler un certain nombre de fois un dé avec un nombre de faces
**\`!snoopify\`** \`phrase\`
    Snoopifies la phrase`,
  useful: `**\`!giphy\`** \`tags du gifs\`
