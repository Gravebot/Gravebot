...
let misc_commands = {
  ...
  quote: (bot, msg, suffix) => {
    let rand = Math.floor(Math.random() * drama.length);
    if (suffix >= 0 || suffix <= 26) {
      bot.sendMessage(msg.channel, quotes[suffix]);
    }
    if (!suffix || isNaN(suffix) || suffix < 0 || suffix > 26) {
