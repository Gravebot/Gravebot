...
function counters(bot, msg, suffix) {
  ...
  return _makeRequest(options)
    ...
    .then(R.prepend(`Sure! Here's the top 10 **statistical** counters for ${champ} ${position}:\n`))
    .then(R.join('\n'))
...
function bans(bot, msg) {
  ...
  return _makeRequest(options)
    ...
    .then(R.slice(0, 10))
    .then(champs => {
      return R.addIndex(R.map)((champ, idx) => `**${_getOrdinal(idx + 1)}**. ${champ}`, champs);
    })
		.tap(console.log)
		.then(R.prepend(`Sure! Here's the top 10 most common bans:\n`))
		.then(R.join('\n'))
		.then(text => bot.sendMessage(msg.channel, text))
    .catch(err => {
      sentry.captureError(err);
