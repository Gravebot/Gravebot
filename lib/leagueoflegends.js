...
// Keep a cache of all champion IDs and Names
const champions = request({url: 'https://ddragon.leagueoflegends.com/cdn/6.1.1/data/en_US/champion.json', json: true})
  .then(R.path(['body', 'data']))
  .then(R.values)
  .then(champs => R.zipObj(R.pluck('key')(champs), R.pluck('name')(champs)));
  ...
  return _makeRiotRequest(`https://${region}.api.pvp.net/api/lol/${region}/v1.3/stats/by-summoner/${summoner.summonerId}/ranked`)
    ...
    .then(champ_stats => {
      ...
      summoner.champion_stats = {
        name: champions.value()[champion_id],
        games: 0,
        winrate: 0
      };
