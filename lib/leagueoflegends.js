...
function _makeRequest(options) {
  let default_options = {
    qs: {api_key: CONFIG.championgg_api},
    json: true
  };
...
function counters(bot, msg, suffix) {
  let query = suffix.replace('counters ', '');
...
function items(bot, msg, suffix) {
  let query = suffix.replace('items ', '');
  let position = R.last(query.split(' '));
  let champ = query.replace(` ${position}`, '');
...
function skills(bot, msg, suffix) {
  let query = suffix.replace('skills ', '');
  let position = R.last(query.split(' '));
  let champ = query.replace(` ${position}`, '');
  ...
  return _makeRequest(options)
    ...
    .then(skills => {
      ...
      let skill_count = R.countBy(R.toLower)(skills);
...
function serverStatus(bot, msg, suffix) {
  let options = {
...
export default {
  lol: (bot, msg, suffix) => {
    if (!CONFIG.championgg_api) {
      return bot.sendMessage(msg.channel, 'Please setup Champion.gg to use the `!lol` command.');
    }
